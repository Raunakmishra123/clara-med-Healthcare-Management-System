...
const Doctors = () => {
  ...
  return (
    <div>
      <p className='text-gray-600'>Browse through our specialists and book your appointment.</p>
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
          {filterDoc.map((item, index) => (
            <div onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='border border-[#C9D8FF] rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-8px] transition-all duration-500' key={index}>
              <img className='bg-[#E8EEFF]' src={item.image} alt="" />
              <div className='p-4'>
